...
def frame_to_ros_msg(cv_frame, downsample, time_header):
...
def parse_times_from_srt(all_srt_full_paths):
    ...
    for srt_file in all_srt_full_paths:
...
def convert_video_to_bag(video_full_path, stamp_list, output_topic, downsample, output_bag_path="/bag-data/saa-data/20201226/output_camera"):
    ...
    with rosbag.Bag(output_bag_path, 'w') as outbag:
        ...
        while cap.isOpened():
            ...
            if status:
                ...
                next_time = stamp_list[frame_number-1]
                ros_msg = frame_to_ros_msg(frame, downsample, next_time)
...
if not os.path.exists(args.video_file_path):
    log.info("Error, path does not seem to exist or access permissions are bad. ").format(args.video_file_path)
    is_fail = True
else:
...
if not os.path.exists(args.closed_caption_path):
    log.info("Error, path does not seem to exist or access permissions are bad. ").format(args.closed_caption_path)
    is_fail = True
else:
...
if not os.path.exists(args.output_path):
    os.path.mkdirs(args.output_path)
...
if False:
    camera_frame_dict = parse_times_from_bag(args.input_bag)
    for camera_frame_topic in camera_frame_dict.keys():
        ...
        convert_video_to_bag(all_video_full_paths[int(camera_id)], camera_frame_dict[camera_frame_topic], camera_frame_topic, args.downsample, args.output_path)

else:
    camera_frame_dict = parse_times_from_srt(all_srt_full_paths)
    for camera_id in camera_frame_dict.keys():
        ...
        convert_video_to_bag(all_video_full_paths[int(camera_id)-1], camera_frame_dict[camera_id], camera_topic_root+camera_id, args.downsample, args.output_path)
