...
def write_frames(bag, writer, total, clahe_obj, topic=None, nframes=repeat(1), start_time=rospy.Time(0), stop_time=rospy.Time(sys.maxint), viz=False, encoding='bgr8'):
    MIN_INTENSITY = 16000
    MAX_INTENSITY = 25000
    bridge = CvBridge()
    if viz:
...
if __name__ == '__main__':
    ...
    for bagfile in glob.glob(args.bagfile):
        ...
        print 'Writing video'
        my_clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8))
        write_frames(bag, writer, len(times), my_clahe, topic=args.topic, nframes=nframes, start_time=args.start, stop_time=args.end, encoding=args.encoding)
        writer.release()
        print '\n'
